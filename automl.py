...
def competition_example_only_rf(input_dir, output_dir, basename, time_budget):
    ...
    train_len = len(D.data['X_train'])
    deleted_lines = train_len * 0.01
    boosting_weights = np.ones(len(D.data['X_train'])) * 1.0 / train_len
    ...
    while time_spent <= time_budget * 0.75 and cycle <= max_cycle:
        ...
        if train_len > 200:
            predictions = M.predict(D.data['X_train'], D.data['Y_train'])
            weighted_error = predictions * boosting_weights
            alpha = (1.0/2) * np.log((1 - weighted_error) * 1.0 / weighted_error)
            boosting_weights = boosting_weights * np.exp(-alpha * D.data['Y_train'] * predictions)
            boosting_weights = boosting_weights * 1.0 / sum(boosting_weights)
            
            indices = boosting_weights.argsort()[:deleted_lines]
            D.data['X_train'] = np.delete(D.data['X_train'], indices, axis=0)
            D.data['Y_train'] = np.delete(D.data['Y_train'], indices, axis=0)
            boosting_weights = np.delete(boosting_weights, indices)
